// Auth Pages
export { default as LoginPage } from './LoginPage';

// Main Pages
export { default as MainPage } from './MainPage';
export { default as DashboardPage } from './DashboardPage';
export { default as ResumesPage } from './resume/ResumesPage';
export { default as CoverLettersPage } from './cover-letter/CoverLettersPage';
export { default as CoverLetterNewPage } from './cover-letter/CoverLetterNewPage';
export { default as CoverLetterViewPage } from './cover-letter/CoverLetterViewPage';
export { default as CoverLetterEditPage } from './cover-letter/CoverLetterEditPage';
export { default as ViewResumePage } from './resume/ViewResumePage';
export { default as EditResumePage } from './resume/EditResumePage';
export { default as PortfolioPage } from './portfolio/PortfolioPage';
export { default as TemplatesPage } from './TemplatesPage';
export { default as CreateResumePage } from './resume/CreateResumePage';
export { default as AboutPage } from './footer/AboutPage';

// Profile Pages
export { default as ProfileEditPage } from './profile/ProfileEditPage';

// Portfolio Pages
export { default as PortfolioCreatePage } from './portfolio/PortfolioCreatePage';
export { default as PortfolioEditPage } from './portfolio/PortfolioEditPage';
export { default as PortfolioViewPage } from './portfolio/PortfolioViewPage';
export { default as UploadPortfolioPage } from './portfolio/UploadPortfolioPage';

// Add UserPage to the exports
export { default as UserPage } from './user/UserPage';

// Footer Section Pages
export { default as SupportPage } from './footer/SupportPage';
export { default as FAQPage } from './footer/FAQPage';
export { default as PrivacyPage } from './footer/PrivacyPage';
export { default as TermsPage } from './footer/TermsPage';
export { default as AcceptableUsePage } from './footer/AcceptableUsePage';
export { default as CopyrightPolicyPage } from './footer/CopyrightPolicyPage';
export { default as AIDataUsePage } from './footer/AIDataUsePage';
export { default as SitePrivacyPage } from './footer/SitePrivacyPage';
export { default as ReportAbusePage } from './footer/ReportAbusePage';
export { default as ContactPage } from './footer/ContactPage';
export { default as WebsitePage } from './WebsitePage';
